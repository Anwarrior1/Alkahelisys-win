...
function SettingsView({ user, canSettings, canUsers, onNotify }: { user: AuthUser; canSettings: boolean; canUsers: boolean; onNotify: (message: ToastMessage) => void }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [nextSettings, nextUsers, nextRoles, nextWorkers] = await Promise.all([
        canSettings ? api.settings() : Promise.resolve(null),
        canUsers ? api.users() : Promise.resolve([]),
        canUsers ? api.roles() : Promise.resolve([]),
        canUsers ? api.workers({ status: 'active' }) : Promise.resolve([]),
      ]);
      if (nextSettings) setSettings((current) => ({ ...current, ...nextSettings }));
      if (canUsers) { setUsers(nextUsers); setRoles(nextRoles); setWorkers(nextWorkers); }
  ...
  return (
    <>
      ...
      {addUserOpen && <CreateUserModal onClose={() => setAddUserOpen(false)} roles={roles} workers={workers} onSaved={(created) => { setUsers((current) => [created, ...current]); setAddUserOpen(false); onNotify({ tone: 'success', text: 'تم إنشاء المستخدم وتعيين دوره.' }); }} onNotify={onNotify} />}
      {editingUser && <EditUserModal user={editingUser} roles={roles} workers={workers} onClose={() => setEditingUser(null)} onSaved={(updated) => { setUsers((current) => current.map((person) => person.id === updated.id ? updated : person)); setEditingUser(null); onNotify({ tone: 'success', text: 'تم تحديث حساب المستخدم.' }); }} onNotify={onNotify} />}
...
function CreateUserModal({ roles, workers, onClose, onSaved, onNotify }: { roles: Role[]; workers: Worker[]; onClose: () => void; onSaved: (user: AuthUser) => void; onNotify: (message: ToastMessage) => void }) {
  const [form, setForm] = useState({ full_name: '', username: '', password: '', role: roles.find((role) => role.key === 'employee')?.key || 'employee', worker_id: '' });
  const [saving, setSaving] = useState(false);
  const update = (key: keyof typeof form, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (form.role === 'employee' && !form.worker_id) { onNotify({ tone: 'error', text: 'اختر العامل المرتبط بحساب الموظف.' }); return; } setSaving(true); try { onSaved(await api.createUser(form)); } catch (error) { onNotify({ tone: 'error', text: friendlyError(error) }); } finally { setSaving(false); } };
  return <Modal title="إضافة مستخدم" subtitle="لا يمكن للمستخدمين تغيير أدوارهم أو ترقيتها بأنفسهم." onClose={onClose}><form className="entry-form" onSubmit={submit}><FormField label="الاسم الكامل" required><input value={form.full_name} onChange={(event) => update('full_name', event.target.value)} required /></FormField><FormField label="اسم المستخدم" required><input value={form.username} onChange={(event) => update('username', event.target.value)} required dir="ltr" /></FormField><FormField label="كلمة مرور أولية" required><input value={form.password} onChange={(event) => update('password', event.target.value)} type="password" minLength={8} required dir="ltr" /></FormField><FormField label="الدور" required><select value={form.role} onChange={(event) => update('role', event.target.value)}>{roles.map((role) => <option key={role.id} value={role.key}>{role.name}</option>)}</select></FormField>{form.role === 'employee' && <FormField label="ملف العامل المرتبط" required hint="يحدد الملف المالي والتشغيلي الذي يمكن لهذا الحساب عرضه."><select value={form.worker_id} onChange={(event) => update('worker_id', event.target.value)} required><option value="">اختر العامل</option>{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select></FormField>}<div className="modal-actions"><Button variant="ghost" onClick={onClose}>إلغاء</Button><Button type="submit" disabled={saving} icon={saving ? <LoaderCircle size={17} className="spin" /> : <UserPlus size={17} />}>{saving ? 'جارٍ الإنشاء...' : 'إنشاء المستخدم'}</Button></div></form></Modal>;
}

function EditUserModal({ user, roles, workers, onClose, onSaved, onNotify }: { user: AuthUser; roles: Role[]; workers: Worker[]; onClose: () => void; onSaved: (user: AuthUser) => void; onNotify: (message: ToastMessage) => void }) {
  const [form, setForm] = useState({ full_name: user.full_name, username: user.username, password: '', role: user.role, status: user.status || 'active', worker_id: user.worker_id || '' });
  const [saving, setSaving] = useState(false);
  const update = (key: keyof typeof form, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (form.password && form.password.length < 10) { onNotify({ tone: 'error', text: 'يجب ألا تقل كلمة المرور الجديدة عن 10 أحرف.' }); return; }
    if (form.role === 'employee' && !form.worker_id) { onNotify({ tone: 'error', text: 'اختر العامل المرتبط بحساب الموظف.' }); return; }
    setSaving(true);
    try { onSaved(await api.updateUser(user.id, { full_name: form.full_name.trim(), username: form.username.trim(), password: form.password || undefined, role: form.role === user.role ? undefined : form.role, status: form.status === user.status ? undefined : form.status, worker_id: form.role === 'employee' ? form.worker_id : undefined })); } catch (error) { onNotify({ tone: 'error', text: friendlyError(error) }); } finally { setSaving(false); }
  };
  return <Modal title="تعديل حساب مستخدم" subtitle="اترك كلمة المرور فارغة إذا لم ترغب في تغييرها." onClose={onClose}><form className="entry-form" onSubmit={submit}><FormField label="الاسم الكامل" required><input value={form.full_name} onChange={(event) => update('full_name', event.target.value)} required /></FormField><FormField label="اسم المستخدم" required><input value={form.username} onChange={(event) => update('username', event.target.value)} required dir="ltr" /></FormField><FormField label="كلمة مرور جديدة"><input value={form.password} onChange={(event) => update('password', event.target.value)} type="password" minLength={10} placeholder="اتركها فارغة دون تغيير" dir="ltr" /></FormField><FormField label="الدور" required><select value={form.role} onChange={(event) => update('role', event.target.value)}>{roles.map((role) => <option key={role.id} value={role.key}>{role.name}</option>)}</select></FormField>{form.role === 'employee' && <FormField label="ملف العامل المرتبط" required hint="لا يستطيع الحساب عرض ملف عامل آخر."><select value={form.worker_id} onChange={(event) => update('worker_id', event.target.value)} required><option value="">اختر العامل</option>{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select></FormField>}<FormField label="حالة الحساب" required><select value={form.status} onChange={(event) => update('status', event.target.value)}><option value="active">نشط</option><option value="disabled">معطل</option></select></FormField><div className="modal-actions"><Button variant="ghost" onClick={onClose}>إلغاء</Button><Button type="submit" disabled={saving} icon={saving ? <LoaderCircle size={17} className="spin" /> : <Save size={17} />}>{saving ? 'جارٍ الحفظ...' : 'حفظ التغييرات'}</Button></div></form></Modal>;
}
