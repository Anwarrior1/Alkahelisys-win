...
function EditWashModal({ wash, workers, showrooms, isManager, onClose, onSaved, onNotify }: { wash: Wash; workers: Worker[]; showrooms: Showroom[]; isManager: boolean; onClose: () => void; onSaved: () => void; onNotify: (message: ToastMessage) => void }) {
  const initialForm = {vehicle_make:wash.vehicle_make,vehicle_model:wash.vehicle_model,manufacturing_year:wash.manufacturing_year?.toString()??'',license_plate:wash.license_plate??'',car_color:wash.car_color??'',wash_type:wash.wash_type??'',price:String(wash.price??''),worker_id:wash.worker_id??'',performed_at:dateTimeInputValue(new Date(wash.performed_at)),payment_type:wash.payment_type,showroom_id:wash.showroom_id??'',showroom_payment_method:wash.showroom_payment_method??(wash.payment_type==='showroom_account'?'cash':'')};
  const [form,setForm]=useState(initialForm);
  const initialOvernight=wash.is_overnight===true;
  const [markAsOvernight,setMarkAsOvernight]=useState(initialOvernight);
  const [saving,setSaving]=useState(false); const update=(key:keyof typeof form,value:string)=>setForm(current=>({...current,[key]:value}));
  const submit=async(event:FormEvent)=>{event.preventDefault();if(form.payment_type==='showroom_account'&&(!form.showroom_id||!['cash','bank'].includes(form.showroom_payment_method))){onNotify({tone:'error',text:'اختر المعرض وطريقة الدفع.'});return;}setSaving(true);try{const operationChanged=(Object.keys(form) as Array<keyof typeof form>).some(key=>form[key]!==initialForm[key]);if(operationChanged){await api.updateWash(wash.id,{...form,manufacturing_year:form.manufacturing_year?Number(form.manufacturing_year):null,license_plate:form.license_plate.trim()||null,car_color:form.car_color.trim()||null,wash_type:form.wash_type.trim()||null,performed_at:form.performed_at===initialForm.performed_at?wash.performed_at:new Date(form.performed_at).toISOString(),showroom_id:form.payment_type==='showroom_account'?form.showroom_id:null,showroom_payment_method:form.payment_type==='showroom_account'?form.showroom_payment_method:null});}if(markAsOvernight!==initialOvernight){await api.setWashOvernight(wash.id,markAsOvernight);}onSaved();}catch(error){onNotify({tone:'error',text:friendlyError(error)});}finally{setSaving(false);}};
      ...
      {loading ? <LoadingBlock label="جارٍ تحميل السجلات المالية المصرح بها..." /> : (
        <>
          ...
          <section className="finance-breakdown glass-card">
            <div className="finance-breakdown__header"><div><p className="eyebrow">حساب الربحية</p><h2>فصل الربح التشغيلي عن المصروفات</h2></div><span className="calculation-note">يُخصم من الربح بعد المصروفات نصيب الأعمال فقط، دون نصيب العمال من المصروف المشترك.</span></div>
            ...
            <div className="calculation-row calculation-row--subtotal"><span>صافي الربح اليومي</span><strong>{money(overview.net_profit_before_expenses)}</strong></div>
